...
public class UserLogInActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent != null) { // 刚才从哪个界面进的，用来判断登陆后是finish还是跳转
            ...
            mIsFromStart = intent.getIntExtra("isFromStart", -1); //如果不等-1说明是从startactivity进入的，那么自动登录后不要finish
        }
        initViews();
        initEvents();
        init();
    ...
    private void haveLogin() {
        ...
        if(!oldPwd.equals("empty")) { //本地有密码
            ...
            } else { //从startactivity进来的，自动登陆后进入相应的mainactivity或selleractivity
                String url = GlobalVariable.URL.LOGIN;
